...
const void store_message_bytes(const std::string &hex_str)
{
    ...
    {
        std::cout << hex_str.substr(j, 2) << std::endl;
...
const void compute_hash()
{
    ...
    for (int i = 1; i <= N; ++i)
    {
        ...
        for (int t = 0; t <= 63; ++t)
        {
            const uint32_t LSIGMA1 = lsigma1(e);
            const uint32_t CH = Ch(e, f, g);
            const uint32_t KKK = K[t];
            const uint32_t WWW = W[t];
            const uint32_t LSIGMA0 = lsigma0(a);
            const uint32_t MAJ = Maj(a, b, c);
